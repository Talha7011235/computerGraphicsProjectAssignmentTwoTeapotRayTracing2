...
PolygonPatch::PolygonPatch(const NffParser* s) : count(0), verticies(), normals(), indicies(), shading(), center(), settings(s) {}
...
int PolygonPatch::getCount() const
{
	return count;
}

std::vector<Vector3> PolygonPatch::getVerticies() const
{
	return verticies;
}

Shading PolygonPatch::getShading() const
{
	return shading;
}

Vector3 PolygonPatch::getCenter() const
{
	return center;
}

void PolygonPatch::setShading(Shading sIsForShading)
{
	shading = sIsForShading;
}

std::istream& operator>> (std::istream& in, PolygonPatch& value)
{
	in >> value.count;
...
bool PolygonPatch::Intersect(const Vector3& origin, const Vector3& direction, float& tValueIsTheDepth,float near, Vector3& uv) const
{
...
bool PolygonPatch::IntersectTriangle(const int vZero, const int vOne, const int vTwo, Vector3 origin, Vector3 direction, float& tValue, Vector3& uv) const
{
...
//  Remove the "hitPoint" as it is not being used.
Vector3 PolygonPatch::getGetHitNormal(const Vector3& hitPoint, const Vector3& uv) const {
	if(settings->isPhongRendering()) {
		Vector3 nA = normals[0];
		Vector3 nB = normals[1];
		Vector3 nC = normals[2];

		Vector3 normal = nA * (1 - uv.getX() - uv.getY()) + nB * uv.getX() + nC * uv.getY();
